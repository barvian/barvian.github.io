...
/**
  ...
  Polymer.NeonPageBehavior = [{
    ...
    attached: function() {
      this._meta = new Polymer.IronMeta({type: 'neon-page-behavior'});
    ...
    _pageChanged: function(event) {
      if (event && event.target === this.parentElement) {
        ...
        if (this.parentElement.selected !== undefined && this.parentElement.selected !== null && this.parentElement.selected.toString() === thisPageValue) {
          ...
          this._meta = new Polymer.IronMeta({type: 'neon-page-behavior', key: 'neon-page-behavior-last-selected', value: thisPageValue});
...
Barvian.PageBehavior = [NeonAnimatableBehavior, NeonPageBehavior, { 
  ...
  _onEntryStart: function _onEntryStart(event) {} }];
/**
...
Polymer({ 
  ...
  _childrenChanged: function _childrenChanged(info) {var _this = this;
    var distributedNodes = Polymer.dom(this.$.content).getDistributedNodes();var 
    length = distributedNodes.length;
    distributedNodes
    // Wrap in li
    .map(function (node) {
    ...
    .forEach(function (node, i) {
      Polymer.dom(i < length / 2 ? _this.$.left : _this.$.right).
      appendChild(node);});}, 
